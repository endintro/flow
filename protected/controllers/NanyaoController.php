...
class NanyaoController extends CController
{
	private $_model;
		...
		if(Yii::app()->getRequest()->getParam("page")) $page = intval(Yii::app()->getRequest()->getParam("page"));	
	 	...
	 	if($page == 1){
	 		if($this->WaterCount <= PAGESIZE){
	 			...
	 			return '<a href="'.Yii::app()->request->getBaseUrl(true).'/nanyao/?page='.($page+1).$keyword.'">下一页 »</a>';
	 		}
	 	}
	 	if($page == $this->PageCount) return '<a href="'.Yii::app()->request->getBaseUrl(true).'/nanyao/?page='.($page-1).$keyword.'">« 上一页</a>';
	 	return '<a href="'.Yii::app()->request->getBaseUrl(true).'/nanyao/?page='.($page-1).$keyword.'">« 上一页</a> | <a href="'.Yii::app()->request->getBaseUrl(true).'/nanyao/?page='.($page+1).$keyword.'">下一页 »</a>';
	}
	/*---------------------page end---------------------------*/
