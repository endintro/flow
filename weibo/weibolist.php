...
/*
$users = allfriends_by_id($c,1571557593);
foreach($users as $user){
	$tags = get_all_tags_by_id($c,$user["id"]);
	print_r($user["name"]);
	sleep(2);
	print_r($tags);
}
...
function get_all_tags_by_id($c,$uid){
	$res = $c->get_tags($uid);
	return $res;
}

function allfriends_by_id($c,$uid){
	$cursor = 0;
	$count  = 200;
	$users = array();
	do{
		if($res = $c->friends_by_id($uid,$cursor,$count)){
